...
app.get('/api/transport', async (req, res) => {
  ...
  try {
    ...
    errors.push(`direct-fetch:${error?.message || String(error)}`);
  }

  try {
    const fallbackData = await fetchPaytagViaDirectApi(payTagId, 'qr', 't');
    if (fallbackData?.transportType && fallbackData?.routeNumber && fallbackData?.vehicleNumber) {
      return res.json({
        vehicleTypeName: fallbackData.transportType,
        routeNumber: fallbackData.routeNumber,
        vehicleNumber: fallbackData.vehicleNumber
      });
    }
    errors.push('fallback-direct-api:empty');
  } catch (error) {
    errors.push(`fallback-direct-api:${error?.message || String(error)}`);
  }

  try {
    const browserData = await fetchPaytagViaHeadlessBrowser(payTagId);
    if (browserData?.transportType && browserData?.routeNumber && browserData?.vehicleNumber) {
      return res.json({
        vehicleTypeName: browserData.transportType,
        routeNumber: browserData.routeNumber,
        vehicleNumber: browserData.vehicleNumber
      });
    }
    errors.push('fallback-browser:empty');
  } catch (error) {
    errors.push(`fallback-browser:${error?.message || String(error)}`);
  }

  return res.status(502).json({ error: `Transport fetch failed (${errors.join(' | ')})` });
});
